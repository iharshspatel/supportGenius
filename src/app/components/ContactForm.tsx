...
export default function ContactForm({ children, className }: ContactFormProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (String(formData.get("website") ?? "")) { setIsSubmitting(false); return; }
    try {
  ...
  return <form onSubmit={handleSubmit} className={className} aria-busy={isSubmitting}><input type="text" name="website" tabIndex={-1} autoComplete="off" aria-hidden="true" className="hidden" />{children}<p aria-live="polite" className="mt-3 text-sm text-[#707070]">{status}</p></form>;
}
